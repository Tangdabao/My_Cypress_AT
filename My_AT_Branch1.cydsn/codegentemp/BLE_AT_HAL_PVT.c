...
void CyBLE_Uart_Start (void)
{
        ...
        /* Setup ISR */
        BLE_AT_uart_isr_StartEx(&CyBLE_Uart_Interrupt);

        BLE_AT_HAL_Uart_Start();
...
void CyBLE_Uart_Stop (void)
{
        ...
        BLE_AT_uart_isr_Stop();

        BLE_AT_HAL_Uart_Stop();
    #endif /* (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART) */
}
...
{
    #if (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART)
        BLE_AT_HAL_Uart_SpiUartPutArray(dataBuf, (uint32)length);
    #endif /* (CYBLE_HCI_TYPE == CYBLE_HCI_OVER_UART) */
}
...
/* Interface to BLE_AT_bless_isr_Enable function */
void CyBleHal_EnableGlobalInterrupts(void)
{
    BLE_AT_bless_isr_Enable();
}

/* Interface to BLE_AT_bless_isr_Disable function */
...
void CyBle_HalInit(void)
{
    ...
    BLE_AT_bless_isr_SetVector(&CyBLE_Bless_Interrupt);
...
void CYBLE_BlessStart(void)
{
    ...
    BLE_AT_bless_isr_StartEx(&CyBLE_Bless_Interrupt);
}
